...
public class TA_CW_CoursePage_SendFile implements Initializable{
    public static final Logger log = LogManager.getLogger(TA_CW_CoursePage_SendFile.class);
        ...
        if(isExam){
            if(exams.get(index).getHomeWorkOrExamType() == HomeWorkOrExamType.FILE){
                ...
                button.setOnAction(event -> {
                    File fileClicked = new File(ReadPropertyFile.passStringFromConfigFile(ConfigIdentifier.publicURL) +  student.getStudentNumber() + "/" + answer);
                    if(fileClicked.exists()){
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setHeaderText("go to this folder: " + ReadPropertyFile.passStringFromConfigFile(ConfigIdentifier.publicURL) + student.getStudentNumber() + "/" + answer);
                        alert.showAndWait();
                        PublicMethods.openADesktopFolder(fileClicked);
                        return;
                    }
                    clientNetwork.sendFileFromServer(answer , student.getStudentNumber());
                });
